-- ============================================================================
-- Retail Watch Store - Fix AI Functions (Drop and Recreate)
-- ============================================================================
-- This script safely drops and recreates all AI functions to avoid overloading errors

USE DATABASE retail_watch_db;
USE SCHEMA public;
USE WAREHOUSE retail_watch_wh;

SELECT '🔧 Fixing AI Function overloading errors...' as fix_step;

-- ============================================================================
-- DROP EXISTING FUNCTIONS FIRST
-- ============================================================================

SELECT '🗑️ Dropping existing AI functions...' as drop_step;
...
DROP FUNCTION IF EXISTS get_customer_360_insights(STRING, STRING);
DROP FUNCTION IF EXISTS get_customer_360_insights(STRING);

SELECT '✅ Existing functions dropped successfully' as drop_status;

-- ============================================================================
-- RECREATE FUNCTIONS WITH SIMPLIFIED VERSIONS
-- ============================================================================

SELECT '🔨 Creating simplified AI functions...' as create_step;

-- ============================================================================
-- 1. CHURN PREDICTION FUNCTION
-- ============================================================================

CREATE FUNCTION predict_customer_churn(customer_id STRING)
RETURNS OBJECT
LANGUAGE SQL
AS
$$
...
CREATE FUNCTION analyze_review_sentiment(review_id STRING)
RETURNS OBJECT
LANGUAGE SQL
AS
$$
...
CREATE FUNCTION optimize_product_pricing(product_id STRING)
RETURNS OBJECT
LANGUAGE SQL
AS
$$
...
CREATE FUNCTION get_personal_recommendations(customer_id STRING, context STRING DEFAULT 'general')
RETURNS OBJECT
LANGUAGE SQL
AS
$$
...
CREATE FUNCTION get_customer_360_insights(customer_id STRING, context STRING DEFAULT 'general')
RETURNS OBJECT
LANGUAGE SQL
AS
$$
...
SELECT '✅ All AI Functions recreated successfully!' as completion_status;
SELECT 'Functions are now free from overloading conflicts and internal errors.' as notice;
SELECT 'You can now run your Streamlit app without function conflicts.' as next_steps; 